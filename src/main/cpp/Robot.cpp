...
void Robot::RobotInit() {
  ...
  shooter->SetDefaultBehaviour(
      [this]() { return wom::make<ShooterManualControl>(shooter, intake, &robotmap.controllers.codriver, _led); });
...
void Robot::RobotPeriodic() {
  ...
  if (shooter->GetState() == ShooterState::kSpinUp && shooter->GetStable() == true) {
    _led->SetState(LEDState::kShooterReady);
  } else if (shooter->GetState() == ShooterState::kSpinUp && shooter->GetStable() == false) {
    _led->SetState(LEDState::kAiming);
  } else if (intake->GetState() == IntakeState::kHold) {
    _led->SetState(LEDState::kHold);
  } else if (intake->GetState() == IntakeState::kIntake) {
...
void Robot::AutonomousInit() {
  ...
  sched->Schedule(autos::make(robotmap._builder, m_autoSelected));
